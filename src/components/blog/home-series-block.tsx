...
export function HomeSeriesBlock({ locale, seriesSlug, posts, totalInSeries }: HomeSeriesBlockProps) {
  if (posts.length === 0) return null;

  return (
    <section className="space-y-5 rounded-2xl border border-white/[0.08] border-l-4 border-l-meta-500/40 bg-dark-800/30 p-6 sm:p-8">
      <div className="space-y-2">
        <p className="label-caps text-meta-400">{t(locale, 'home.seriesEyebrow')}</p>
        <h2 className="font-display text-2xl font-medium text-ink">
          {formatSeriesName(seriesSlug, locale)}
        </h2>
        <p className="text-sm text-ink-muted">{seriesArticleCountLabel(locale, totalInSeries)}</p>
      </div>

      <ol className="space-y-3">
        {posts.map((post) => {
          const order = post.series_order ?? 0;
          return (
            <li key={post.id}>
              <Link
                href={`/blog/${post.slug}`}
              ...
              >
                <span className="mt-0.5 font-mono text-xs text-meta-400">
